...
public class ProducerDemoKey {
    ...
    public static void main(String[] args) {
        ...
        // send data
        for(int i=0;i<30;i++){
            ProducerRecord<String,String> producerRecord = new ProducerRecord<>("demo_java1", "id"+i, "Hnowhere"+i);
            log.info("Producer partitio before "+producerRecord.partition());
            producer.send(producerRecord, (RecordMetadata metadata, Exception exception)-> {
                if(exception == null)
                    log.info("Receievd metadata"+ metadata.partition() +","+ metadata.partition()+","+producerRecord.key());
                else
